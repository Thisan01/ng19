...
Asiata.addcommand({pattern: 'plug ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {
...
Asiata.addcommand({pattern: 'myplugin', fromMe: true, desc: Lang.PLUGIN_DESC }, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('Thisan01') ? msg : inmsg
...
Asiata.addcommand({pattern: 'unplug(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
