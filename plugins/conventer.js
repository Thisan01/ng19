...
if (Config.WORKTYPE == 'private') {
    ...
    Asiata.addcommand({pattern: 'photo', fromMe: true,  deleteCommand: false, desc: Lang.STİCKER_DESC}, (async (message, match) => {   
        ...
        ffmpeg(location)
            ...
            .save('Asiata.jpg')
            .on('end', async () => {
                await message.client.sendMessage(message.jid, fs.readFileSync('Asiata.jpg'), MessageType.image, {quoted: message.data}, {mimetype: Mimetype.jpg , Caption : '▰✖🧚‍♂️ꜱʟ ᴀꜱɪᴀᴛᴀ ʙᴏᴛ🌟✖▰'});
            });
        return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
    }));
}
