...
if (Config.WORKTYPE == 'private') {
    Asiata.addcommand({pattern: 'sticker$', fromMe: true, desc: Lang.STICKER_DESC,  deleteCommand: false}, (async (message, match) => {    
    ...
    Asiata.addcommand({pattern: 'sticvid$', fromMe: true, desc: Lang.ANI_STICK,  deleteCommand: false}, (async (message, match) => {
        ...
        await webp2mp4File(savedFilename).then(async (rest) => {
            await Axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
                ...
                saving.on("finish", async () => {
                    await message.client.sendMessage(msgid, fs.readFileSync('/root/ng19/vstic.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: '▰✖🧚‍♂️ꜱʟ ᴀꜱɪᴀᴛᴀ ʙᴏᴛ🌟✖▰   ', quoted: message.data })
     ...
     Asiata.addcommand({pattern: 'sticker$', fromMe: false, desc: Lang.STICKER_DESC}, (async (message, match) => {    
    ...
    Asiata.addcommand({pattern: 'sticvid$', fromMe: false, desc: Lang.ANI_STICK}, (async (message, match) => {
        const msgid = message.jid
        if (message.reply_message === false) return await message.sendMessage(Lang.ANI_REPLY);
        await message.client.sendMessage(msgid, Lang.ANIMATE, MessageType.text, {quoted: message.data})
        const savedFilename = await message.client.downloadAndSaveMediaMessage({
