...
const {Message, StringSession, Image, Video} = require('./Asiata/');
...
// ════════════════════SQL🇱🇰🇱🇰
const AsiataDB = config.DATABASE.define('Asiata', {
    info: {
...
// ════════════════════WA CONNECTION🇱🇰🇱🇰🇱🇰
    ...
    conn.on ('credentials-updated', async () => {
        ...
        if (StrSes_Db.length < 1) {
            await AsiataDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
...
// ════════════════════PLUGGINS SUCCESS🇱🇰🇱🇰🇱🇰
        console.log(
            chalk.green.bold('☃️❄️ ASIATA  WHATSAPP BOT WORKING!▷')
       );
