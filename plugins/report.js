...
if (Config.WORKTYPE == 'private') {
    Asiata.addcommand({pattern: 'report ?(.*)', fromMe: true, desc: Lang.REPORT,  deleteCommand: false}, (async (message, match) => {
    ...
    Asiata.addcommand({pattern: 'report ?(.*)', fromMe: false, desc: Lang.REPORT}, (async (message, match) => {
        if (match[1] == '' && message.reply_message) {
            let grp = await message.client.groupMetadata(message.jid);
            var jids = [];
            msg = '';
