...
if (Config.CHAT_BOT == 'true') {
	if (Config.WORKTYPE == 'private') {
		Asiata.addcommand({pattern: 'sl ?(.*)', fromMe: true,  deleteCommand: false,  desc: Lang.BOT_DESC}, async (message, match) => {
		...
		Asiata.addcommand({pattern: 'sl ?(.*)', fromMe: false, desc: Lang.BOT_DESC}, async (message, match) => {
			if (match[1] === 'xx') return await message.reply(Lang.NEED_LOCATIONA);
			const url = `https://api.simsimi.net/v1/?text=${match[1]}&lang=en&cf=true`;
			try {
				const response = await got(url);
...
Asiata.addcommand({pattern: 'switch ?(.*)', fromMe: true,  deleteCommand: false,  desc: Lang.SWITCH_DESC}, (async(message, match) => {
